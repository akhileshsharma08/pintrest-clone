...
const CommonPins = () => {
  ...
  return (
    <div className="mt-7 px-2 md:px-5 columns-2 md:columns-3 lg:columns-4 mb-4 xl:columns-5 space-y-3 mx-auto">
      {Array.isArray(PostDetails) &&
        PostDetails.map((item, index) => (
          <Link href={`/pindetail/${item._id}`}>
          <div
            className="relative rounded-3xl mb-2 cursor-pointer hover:bg-gray-800 hover:scale-105 transition-all ease-in"
            key={index}
          >
            <Image
              ...
              width={500}
              height={500}
              className="rounded-3xl cursor-pointer relative z-0 w-full hover:opacity-70"
            />
